...
class Connection():
    def __init__(self,host,port,start_seq,debug=False,binaMode=False):
        self.debug = debug
...
class Receiver():
    def __init__(self,listenport=33122,debug=False,timeout=10, sackMode=False, binaMode=False):
        self.debug = debug
    ...
    def start(self):
        while True:
            try:
                ...
                if (not self.binaMode and Checksum.validate_checksum(message)) \
                    or (self.binaMode and Checksum.validate_checksum_bina(message)):
    ...
    def _send_ack(self, seqno, address):
        if self.sackMode:
            ...
            m = "ack|%s|" % seqno
        if self.binaMode:
            checksum = Checksum.generate_checksum_bina(m.encode())
        else:
    ...
    def _handle_start(self, seqno, data, address):
        if not address in self.connections:
            self.connections[address] = Connection(address[0],address[1],seqno,self.debug, self.binaMode)
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],
                               "p:dt:kb", ["port=", "debug=", "timeout=", "sack=", "bina="])
    except:
        usage()
        exit()
    ...
    for o,a in opts:
        if o in ("-p", "--port="):
            ...
            usage()
            exit()
    r = Receiver(port, debug, timeout, sackMode, binaMode)
    r.start()